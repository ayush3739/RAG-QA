...
{doc_context}"""
    ...
    return (
        ...
        "- For web/news questions, summarize the concrete facts from WEB CONTEXT. "
        "Do not answer with only a list of websites to visit.\n"
        "- If the user asks for details from prior pages/sources/links, use the "
        "provided WEB CONTEXT from history and do not perform generic advice.\n"
        "- Include links only when useful, but keep the linked text readable."
        + doc_names_str + "\n\n"
        f"DOCUMENT CONTEXT:\n{doc_context or 'None'}\n\n"
        f"WEB CONTEXT:\n{web_context or 'None'}"
    )
