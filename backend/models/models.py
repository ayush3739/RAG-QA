...
class User(Base):
    ...
    sessions : Mapped[list[Session]] = relationship(back_populates="id", cascade="all, delete-orphan")
...
class Session(Base):
    ...
    chat_documents: Mapped[list["Chat_Document"]] = relationship(back_populates="chat", cascade="all, delete-orphan")
...
class Message(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    chat_id: Mapped[int] = mapped_column(ForeignKey("sessions.id"), nullable=False, index=True)
    ...
    chat: Mapped["Session"] = relationship(back_populates="messages")
...
class Document(Base):
    ...
    chat_documents: Mapped[list["Chat_Document"]] = relationship(back_populates="document", cascade="all, delete-orphan")


class Chat_Document(Base):
    __tablename__ = "chat_documents"
    id: Mapped[str] = mapped_column(String(36), primary_key=True, index=True)
    chat_id: Mapped[str] = mapped_column(ForeignKey("sessions.id"), nullable=False, index=True)
    document_id: Mapped[str] = mapped_column(ForeignKey("documents.id"), nullable=False, index=True)
    added_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=lambda: datetime.now(UTC))

    chat: Mapped["Session"] = relationship(back_populates="chat_documents")
